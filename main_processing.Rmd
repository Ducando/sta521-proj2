...
```{r}
## F1 ESTIMATES
image_log_recall <- image_log$conf_mat["predict_1", "truth_1"] / (image_log$conf_mat["predict_1", "truth_1"] + image_log$conf_mat["predict_-1", "truth_1"] )
image_rf_recall <- image_rf$conf_mat["predict_1", "truth_1"] / (image_rf$conf_mat["predict_1", "truth_1"] + image_rf$conf_mat["predict_-1", "truth_1"] )
image_lda_recall <- image_lda$conf_mat["predict_1", "truth_1"] / (image_lda$conf_mat["predict_1", "truth_1"] + image_lda$conf_mat["predict_-1", "truth_1"] )
image_qda_recall <- image_qda$conf_mat["predict_1", "truth_1"] / (image_qda$conf_mat["predict_1", "truth_1"] + image_qda$conf_mat["predict_-1", "truth_1"] )
image_boost_recall <- image_boost$conf_mat["predict_1", "truth_1"] / (image_boost$conf_mat["predict_1", "truth_1"] + image_boost$conf_mat["predict_-1", "truth_1"] )

block_log_recall <- block_log$conf_mat["predict_1", "truth_1"] / (block_log$conf_mat["predict_1", "truth_1"] + block_log$conf_mat["predict_-1", "truth_1"] )
block_rf_recall <- block_rf$conf_mat["predict_1", "truth_1"] / (block_rf$conf_mat["predict_1", "truth_1"] + block_rf$conf_mat["predict_-1", "truth_1"] )
block_lda_recall <- block_lda$conf_mat["predict_1", "truth_1"] / (block_lda$conf_mat["predict_1", "truth_1"] + block_lda$conf_mat["predict_-1", "truth_1"] )
block_qda_recall <- block_qda$conf_mat["predict_1", "truth_1"] / (block_qda$conf_mat["predict_1", "truth_1"] + block_qda$conf_mat["predict_-1", "truth_1"] )
block_boost_recall <- block_boost$conf_mat["predict_1", "truth_1"] / (block_boost$conf_mat["predict_1", "truth_1"] + block_boost$conf_mat["predict_-1", "truth_1"] )
...
image_f <- round(c(image_log_f, image_rf_f,image_boost_f,image_lda_f,image_qda_f),4)
block_f <- round(c(block_log_f, block_rf_f,block_boost_f,block_lda_f,block_qda_f),4)
models <- c("Logistic Regression", "Random Forest", "Boosted Trees", "LDA", "QDA")
...
ggplot(block_accuracy, aes(mtry,min_node,fill=Accuracy)) + 
  geom_tile() +theme_light()+scale_y_continuous(breaks = c(2,4,6,8),labels=c(5,15,25,35))+
...
```{r}
rf_mod <- ranger(expert ~., data = rf_train_block,mtry = 5, min_n = 25, trees = 100)
preds <- predict(mod,data=rf_val_block)
mean(rf_val_block$expert==preds$predictions)
```

```{r}
rf_mod <- ranger(expert ~., data = rf_train_block,mtry = 5, min_n = 25, trees = 250)
preds <- predict(mod,data=rf_val_block)
mean(rf_val_block$expert==preds$predictions)
```

```{r}
rf_mod <- ranger(expert ~., data = rf_train_block,mtry = 5, min_n = 25, trees = 500)
preds <- predict(mod,data=rf_val_block)
mean(rf_val_block$expert==preds$predictions)
```
...
```{r}
set.seed(513)
final_mod <- ranger(expert ~., data = rf_train_block,mtry = 5, min_n = 25, trees = 50,importance = 'impurity')
...
```{r}
final_mod_prob <- ranger(expert ~., data = rf_train_block,mtry = 5, min_n = 25, trees = 50,probability = TRUE)
...
image_1_pred_class <- image_1_pred_class %>%
  mutate(misclassification = ifelse(imagem1$expert != 0 & class != imagem1$expert,"Yes","No"))
...
image_2_pred_class <- image_2_pred_class %>%
  mutate(misclassification = ifelse(imagem2$expert != 0 & class != imagem2$expert,"Yes","No"))
...
image_3_pred_class <- image_3_pred_class %>%
  mutate(misclassification = ifelse(imagem3$expert != 0 & class != imagem3$expert,"Yes","No"))
...
Based on this, pick 2 for mtry and 35 for min node (note accuracy is significantly worse), 0.6800	

```{r}
set.seed(513)
final_image_mod <- ranger(expert ~., data = rf_train_image,mtry = 2, min_n = 35, trees = 50,importance = 'impurity')
