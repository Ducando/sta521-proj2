...
Using the first way of splitting the data by image
Looks like LDA is performing best with this way 
```{r}
# LDA 
...
lda_tester %>%
  ...
  ggplot(aes(m = probs.1, d = ylabels, color = as.factor(fold))) + geom_roc( labels = FALSE)
```
...
ROC 
need to return the prediction and labels for each in the function
and then use that to calculate
the roc and then the mean one for each of the model methods 
