...
imagem1 %>%
  ...
  mutate(expert_text = ifelse(expert == 0, "unlabeled", 
                              ifelse(expert == 1, "cloudy","clear"))) -> imagem1

imagem2 %>%
  mutate(expert = as.factor(expert)) %>%
  mutate(expert_text = ifelse(expert == 0, "unlabeled", 
                              ifelse(expert == 1, "cloudy","clear"))) -> imagem2

imagem3 %>%
  mutate(expert = as.factor(expert)) %>%
  mutate(expert_text = ifelse(expert == 0, "unlabeled", 
                              ifelse(expert == 1, "cloudy","clear"))) -> imagem3
...
p1 <- ggplot(imagem1, aes(x = x_coord, y = y_coord, color = expert_text)) +
  ...
  scale_colour_manual(labels = c("Clear", "Cloudy", "Unlabeled"),
                      values = c("#132B43", "#56B1F7", "purple")) +
...
p1 <- imagem1 %>%
  ...
  geom_histogram() +
  scale_fill_manual(labels = c("Clear", "Cloudy"), 
...
```{r}
split_blocks_main <- function(df, num_blocks, train_num_blocks, val_num_blocks){
  blocks <- find_blocks(df$x_coord, df$y_coord, num_blocks)
  block_indices <- split_blocks(blocks, num_blocks, 
...
```{r}
image1_dfs <- split_blocks_main(imagem1, 16, 10, 3)
image2_dfs <- split_blocks_main(imagem2, 16, 10, 3)
image3_dfs <- split_blocks_main(imagem3, 16, 10, 3)
...
This classifier will have high average accuracy when the image is mostly 
cloud free.
...
names <- c("NDAI", "SD", "CORR", 
           "Angle DF", "Angle CF", "Angle BF", "Angle AF", "Angle AN")
...
NDAI and CORR are selected as best features becuase they have the highest 
correlation values with the expert labels. SD is also selected because the
range of values between the cloudy and cloud free data is more separable than 
the range of values for the angles.
...
model_sum %>%
  ...
  pivot_longer(cols = c(Penalty, `Min Try / Min Nodes`), names_to = "Parameter", 
               values_to = "Value")  %>%
  #pivot wider by type 
  filter(!is.na(Value)) %>%
...
```{r}
## PRECISION ESTIMATES 
image_precision <- round(c(image_log$precision, image_rf$precision,
                           image_boost$precision,image_lda$precision,
                           image_qda$precision),4)
block_precision <- round(c(block_log$precision, block_rf$precision,
                           block_boost$precision,block_lda$precision,
                           block_qda$precision),4)
models <- c("Logistic Regression", "Random Forest", 
            "Boosted Trees", "LDA", "QDA")
...
```{r}
## F1 ESTIMATES
image_log_recall <- image_log$conf_mat["predict_1", "truth_1"] / 
  (image_log$conf_mat["predict_1", "truth_1"] + 
     image_log$conf_mat["predict_-1", "truth_1"] )
image_rf_recall <- image_rf$conf_mat["predict_1", "truth_1"] / 
  (image_rf$conf_mat["predict_1", "truth_1"] + 
     image_rf$conf_mat["predict_-1", "truth_1"] )
image_lda_recall <- image_lda$conf_mat["predict_1", "truth_1"] / 
  (image_lda$conf_mat["predict_1", "truth_1"] + 
     image_lda$conf_mat["predict_-1", "truth_1"] )
image_qda_recall <- image_qda$conf_mat["predict_1", "truth_1"] / 
  (image_qda$conf_mat["predict_1", "truth_1"] +
     image_qda$conf_mat["predict_-1", "truth_1"] )
image_boost_recall <- image_boost$conf_mat["predict_1", "truth_1"] / 
  (image_boost$conf_mat["predict_1", "truth_1"] + 
     image_boost$conf_mat["predict_-1", "truth_1"] )

block_log_recall <- block_log$conf_mat["predict_1", "truth_1"] / 
  (block_log$conf_mat["predict_1", "truth_1"] + 
     block_log$conf_mat["predict_-1", "truth_1"] )
block_rf_recall <- block_rf$conf_mat["predict_1", "truth_1"] / 
  (block_rf$conf_mat["predict_1", "truth_1"] + 
     block_rf$conf_mat["predict_-1", "truth_1"] )
block_lda_recall <- block_lda$conf_mat["predict_1", "truth_1"] / 
  (block_lda$conf_mat["predict_1", "truth_1"] + 
     block_lda$conf_mat["predict_-1", "truth_1"] )
block_qda_recall <- block_qda$conf_mat["predict_1", "truth_1"] / 
  (block_qda$conf_mat["predict_1", "truth_1"] + 
     block_qda$conf_mat["predict_-1", "truth_1"] )
block_boost_recall <- block_boost$conf_mat["predict_1", "truth_1"] / 
  (block_boost$conf_mat["predict_1", "truth_1"] + 
     block_boost$conf_mat["predict_-1", "truth_1"] )

# recall = TP / TP + FN
# F1 = 2 * 1 / ((1 / precision) + (1/recall))

image_log_f <- 2 * 1 / ((1 / image_log$precision) + (1 / image_log_recall))
image_rf_f <- 2 * 1 / ((1 / image_rf$precision) + (1 / image_rf_recall))
image_boost_f <- 2 * 1 / ((1 / image_boost$precision) + (1 /image_boost_recall))
image_lda_f <- 2 * 1 / ((1 / image_lda$precision) + (1 / image_lda_recall))
image_qda_f <- 2 * 1 / ((1 / image_qda$precision) + (1 / image_qda_recall))

block_log_f <- 2 * 1 / ((1 / block_log$precision) + (1 / block_log_recall))
block_rf_f <- 2 * 1 / ((1 / block_rf$precision) + (1 / block_rf_recall))
block_boost_f <- 2 * 1 / ((1 / block_boost$precision) + (1 /block_boost_recall))
block_lda_f <- 2 * 1 / ((1 / block_lda$precision) + (1 / block_lda_recall))
block_qda_f <- 2 * 1 / ((1 / block_qda$precision) + (1 / block_qda_recall))

image_f <- round(c(image_log_f, image_rf_f,
                   image_boost_f,image_lda_f,image_qda_f),4)
block_f <- round(c(block_log_f, block_rf_f
                   ,block_boost_f,block_lda_f,block_qda_f),4)
models <- c("Logistic Regression", "Random Forest", 
            "Boosted Trees", "LDA", "QDA")

bind_cols(Model = models, `Image Split` = image_f) %>%
  bind_cols(`Block Split` = block_f) %>%
  arrange(desc(`Block Split`)) -> f1_table
...
heatmap(block_acc, Colv = NA, Rowv = NA,
...
ggplot(block_accuracy, aes(mtry,min_node,fill=Accuracy)) + 
  geom_tile() +theme_light()+scale_y_continuous(breaks = c(2,4,6,8),
                                                labels=c(5,15,25,35))+
  theme(axis.text = element_text(size=9), axis.title = element_text(size = 9), 
        legend.title = element_text(size = 9), 
        plot.title = element_text(size = 12, hjust = 0.5))+
  labs(x="Number of Variables Available",y="Minimum Node Size", 
       title = "Validation Accuracy")
```
Based on this, picking 4 for mtry and 25 for min_n (0.9139698)

```{r}
rf_mod <- ranger(expert ~., data = rf_train_block,mtry = 5,
                 min_n = 25, trees = 100)
preds <- predict(mod,data=rf_val_block)
...
```{r}
set.seed(513)
final_mod <- ranger(expert ~., data = rf_train_block,mtry = 5, 
                    min_n = 25, trees = 50,importance = 'impurity')
...
```{r}
final_mod_prob <- ranger(expert ~., data = rf_train_block,mtry = 5, 
                         min_n = 25, trees = 50,probability = TRUE)
test_prob_preds <- predict(final_mod_prob,data=rf_test_block)
```

```{r}
var_imp <- tibble(variable=names(final_mod$variable.importance),
                  importance = final_mod$variable.importance)
...
image_1_pred_class <- tibble(x_coord = imagem1$x_coord, 
                             y_coord = imagem1$y_coord, class = image_1_class)
...
image_1_probs <- tibble(x_coord = imagem1$x_coord, y_coord = imagem1$y_coord,
                        Probability = image_1_preds[,2])
ggplot(image_1_probs, aes(x_coord,y_coord, fill = Probability)) + 
     geom_raster()+labs(x="X Coordinate", y="Y Coordinate", 
                        title = "Image 1 Cloudiness Probability")+
  theme_light()+
...
image_2_pred_class <- tibble(x_coord = imagem2$x_coord, 
                             y_coord = imagem2$y_coord, class = image_2_class)
...
image_2_probs <- tibble(x_coord = imagem2$x_coord, 
                        y_coord = imagem2$y_coord,
                        Probability = image_2_preds[,2])
...
image_3_pred_class <- tibble(x_coord = imagem3$x_coord, 
                             y_coord = imagem3$y_coord, class = image_3_class)
...
image_3_probs <- tibble(x_coord = imagem3$x_coord, 
                        y_coord = imagem3$y_coord, 
                        Probability = image_3_preds[,2])
...
image_3_plot <- ggplot(image_3_probs, aes(x_coord,y_coord, 
                                          fill = Probability)) + 
  ...
  theme(axis.text = element_text(size=10), axis.title = element_text(size = 10), 
        legend.title = element_text(size = 12), 
        plot.title = element_text(size = 12, hjust = 0.5))
image_1_plot + image_2_plot + image_3_plot + plot_layout(guides = "collect") & 
  theme(legend.position = 'bottom')
```
...
image_1_pred_class <- image_1_pred_class %>%
  mutate(misclassification = ifelse(imagem1$expert != 0 & 
                                      class != imagem1$expert,"Yes","No"))

ggplot(image_1_pred_class, aes(x_coord,y_coord, fill = misclassification)) + 
     geom_raster()+ scale_fill_manual(values = c("#132B43", "#56B1F7"))
```

```{r}
image_2_pred_class <- image_2_pred_class %>%
  mutate(misclassification = ifelse(imagem2$expert != 0 & 
                                      class != imagem2$expert,"Yes","No"))

ggplot(image_2_pred_class, aes(x_coord,y_coord, fill = misclassification)) + 
     geom_raster()+ scale_fill_manual(values = c("#132B43", "#56B1F7"))
```

```{r}
image_3_pred_class <- image_3_pred_class %>%
  mutate(misclassification = ifelse(imagem3$expert != 0 & 
                                      class != imagem3$expert,"Yes","No"))

ggplot(image_3_pred_class, aes(x_coord,y_coord, fill = misclassification)) + 
     geom_raster()+ scale_fill_manual(values = c("#132B43", "#56B1F7"))
```
...
image_1_pred_class <- image_1_pred_class %>%
  mutate(misclassification = ifelse(imagem1$expert != 0 & 
                                      class != imagem1$expert,1,.2))
image_2_pred_class <- image_2_pred_class %>%
  mutate(misclassification = ifelse(imagem2$expert != 0 & 
                                      class != imagem2$expert,1,.2))
image_3_pred_class <- image_3_pred_class %>%
  mutate(misclassification = ifelse(imagem3$expert != 0 & 
                                      class != imagem3$expert,1,.2))
```


```{r,fig.width=7,fig.height=3}
misclass_1 <- ggplot(image_1_pred_class, 
                     aes(x_coord,y_coord, fill = class,alpha=misclassification)) + 
  geom_raster()+ 
  scale_fill_manual(labels = c("Clear", "Cloudy"), 
                    values = c("#132B43", "#56B1F7"))+ 
  scale_alpha_identity()+
  geom_hline(yintercept = 2,alpha=0.5)+
  geom_hline(yintercept = 97,alpha=0.5)+geom_hline(yintercept = 192,alpha=0.5)+
  geom_hline(yintercept = 287,alpha=0.5)+geom_hline(yintercept = 383,alpha=0.5)+
  geom_vline(xintercept=65,alpha=0.5)+
  geom_vline(xintercept=140,alpha=0.5)+geom_vline(xintercept=216,alpha=0.5)+
...
misclass_2 <- ggplot(image_2_pred_class, 
                     aes(x_coord,y_coord, fill = class,alpha=misclassification)) + 
  geom_raster()+ 
  scale_fill_manual(labels = c("Clear", "Cloudy"), 
                    values = c("#132B43", "#56B1F7"))+ scale_alpha_identity()+
  geom_hline(yintercept = 2,alpha=0.5)+geom_hline(yintercept = 97,alpha=0.5)+
  geom_hline(yintercept = 192,alpha=0.5)+
  geom_hline(yintercept = 287,alpha=0.5)+geom_hline(yintercept = 383,alpha=0.5)+
  geom_vline(xintercept=65,alpha=0.5)+geom_vline(xintercept=141,alpha=0.5)+
  geom_vline(xintercept=217,alpha=0.5)+
...
misclass_3 <- ggplot(image_3_pred_class, 
                     aes(x_coord,y_coord, 
                         fill = class,alpha=misclassification)) + 
  geom_raster() + scale_fill_manual(labels = c("Clear", "Cloudy"), 
                                    values = c("#132B43", "#56B1F7"))+ 
  scale_alpha_identity()+
  geom_hline(yintercept = 2,alpha=0.5)+geom_hline(yintercept = 97,alpha=0.5)+
  geom_hline(yintercept = 192,alpha=0.5)+
  ...
  theme(axis.text.y = element_blank(),axis.ticks.y = element_blank(),
        ...
        axis.title.x = element_blank(), 
        panel.grid.major = element_blank(),panel.grid.minor = element_blank())
misclass_1 + misclass_2 + misclass_3 + plot_layout(guides = "collect") & 
  theme(legend.position = 'bottom')
```
...
for (i in seq_along(mtrys)){
  for (j in seq_along(min_ns)){
    mod <- ranger(expert ~., data = rf_train_image,mtry = mtrys[i],
                  min_n = min_ns[j], trees = 50)
...
Based on this, pick 2 for mtry and 35 for min node 
(note accuracy is significantly worse), 0.6800	

```{r}
set.seed(513)
final_image_mod <- ranger(expert ~., data = rf_train_image,mtry = 2, 
                          min_n = 35, trees = 50,importance = 'impurity')
...
```{r}
var_imp_image <- tibble(variable=names(final_image_mod$variable.importance),
                        importance = final_image_mod$variable.importance)

 ggplot(var_imp_image, aes(x=reorder(variable,importance), 
...
image_1_image_class <- tibble(x_coord = imagem1$x_coord, 
                              y_coord = imagem1$y_coord, 
                              class = image_1_image_preds)
image_1_image_class <- image_1_image_class %>%
  mutate(misclassified = ifelse(imagem1$expert != 0 & 
                                  class != imagem1$expert,"Yes","No"))
...
image_2_image_class <- tibble(x_coord = imagem2$x_coord, 
                              y_coord = imagem2$y_coord, 
                              class = image_2_image_preds)
image_2_image_class <- image_2_image_class %>%
  mutate(misclassified = ifelse(imagem2$expert != 0 & 
                                  class != imagem2$expert,"Yes","No"))
...
image_3_image_class <- tibble(x_coord = imagem3$x_coord, 
                              y_coord = imagem3$y_coord,
                              class = image_3_image_preds)
image_3_image_class <- image_3_image_class %>%
  mutate(misclassified = ifelse(imagem3$expert != 0 & 
                                  class != imagem3$expert,"Yes","No"))
